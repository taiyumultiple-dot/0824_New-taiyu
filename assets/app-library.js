...
function render(){const visible=resources.filter(resource=>(scope==='all'||resource.scope===scope)&&(type==='all'||resource.type===type));resourceEl.innerHTML=visible.map(card).join('');countEl.textContent=`${visible.length} 份教材`;titleEl.textContent=scopeTitle();document.querySelector('#empty').hidden=visible.length>0}
document.querySelectorAll('.scope-tab').forEach(button=>button.onclick=()=>{scope=button.dataset.scope;document.querySelectorAll('.scope-tab').forEach(item=>{const selected=item===button;item.classList.toggle('is-active',selected);item.setAttribute('aria-pressed',selected)});render()});
document.querySelectorAll('.tab').forEach(button=>button.onclick=()=>{type=button.dataset.filter;document.querySelectorAll('.tab').forEach(item=>{const selected=item===button;item.classList.toggle('is-active',selected);item.setAttribute('aria-pressed',selected)});render()});
render();
